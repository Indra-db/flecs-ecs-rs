...
//! (spec §3.4 / GAP-3): a lone optional is a valid `get_ref` shape, not only
...
fn option_ref_present_is_some_some_and_locks_read() {
    ...
    let g = e.get_ref::<Option<&Position>>().unwrap();
...
fn option_ref_absent_is_some_none_and_locks_nothing() {
    ...
    let a = e.get_ref::<Option<&Velocity>>().unwrap();
    assert!(a.is_none());
    ...
    let b = e.get_ref::<Option<&mut Velocity>>().unwrap();
    assert!(b.is_none());
    drop((a, b));
}
...
fn option_mut_present_is_some_some_and_mutates() {
    ...
    {
        let mut g = e.get_ref::<Option<&mut Position>>().unwrap();
        g.as_mut().unwrap().x += 10;
    }
    assert_eq!(e.get_ref::<&Position>().unwrap().x, 11);
}
...
fn option_mut_present_locks_write() {
    ...
    let g = e.get_ref::<Option<&mut Position>>().unwrap();
    // A live write blocks a read of the same component.
    assert!(matches!(
        e.try_get_ref::<&Position>(),
