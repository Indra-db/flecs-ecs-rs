...
fn query_uncached_destruction_no_panic() {
    ...
    drop(query);
    query2.run_exclusive(&mut world, |mut it| while it.next() {});
    drop(query2);
}
...
fn query_cached_destruction_lingering_references_panic() {
    ...
    let query2 = query.clone();
    query.destruct();
    query2.run_exclusive(&mut world, |_| {});
    drop(query2);
}
...
fn query_cached_destruction_sole_owner_no_use_after_free() {
    ...
    query.each_entity_exclusive(&mut world, |entity, _| {
...
fn query_handle_keeps_query_alive() {
    ...
    handle
        .iter_stage(&world)
        .each_shared(&world, |_| count.set(count.get() + 1));
    assert_eq!(count.get(), 1);
}
...
fn query_handle_entity_query_off_thread_drop() {
    ...
    let count = core::cell::Cell::new(0);
    query.each_exclusive(&mut world, |_| count.set(count.get() + 1));
    assert_eq!(count.get(), 1);
    query.destruct();
}
...
fn query_iter_stage_wrong_world_panics() {
    ...
    query.iter_stage(&world_b).each_shared(&world_b, |_| {});
}
...
fn query_handle_iter_stage_wrong_world_panics() {
    ...
    handle.iter_stage(&world_b).each_shared(&world_b, |_| {});
}
...
fn query_panic_inside() {
    ...
    query.run_exclusive(&mut world, |_| {
        panic!();
    });
}
...
fn query_term_each_pair_type() {
    ...
    let q = world.query::<()>().with((Rel2::id(), Obj2::id())).build();
    q.each_entity_shared(&world, |e, ()| {
        if e == e1 || e == e2 || e == e3 {
            count += 1;
        }
...
fn query_default_ctor() {
    ...
    let mut count = 0;
    q_var.as_ref().unwrap().each_entity_exclusive(&mut world, |_e, p| {
        assert_eq!(p.x, 10);
        assert_eq!(p.y, 20);
        count += 1;
    });
...
fn query_term_get_id() {
    ...
    assert_eq!(
        world.id_view_from(q.term(2).id()).second_id().id(),
        bar.id()
    );
}
...
fn query_set_this_var() {
    ...
    let mut count = 0;
    q.with_var(0, e2).each_entity_shared(&world, |e, _| {
        assert_eq!(e, e2);
        count += 1;
    });
    assert_eq!(count, 1);
...
fn query_changed() {
    ...
    q_w.each_exclusive(&mut world, |_p| {});
    assert!(q.is_changed());
}
...
fn query_compare_term_id() {
    ...
    q.run_shared(&world, |mut it| {
        while it.next() {
            assert_eq!(*it.id(0).entity_view().id(), Tag::entity_id(&world));
...
fn query_each_pair_type() {
    ...
    let e1 = world
        .entity()
        .set_pair::<EatsData, ApplesTag>(EatsData { amount: 10 });
    world
        .entity()
        .set_pair::<EatsData, PearsTag>(EatsData { amount: 20 });

    let q = world.new_query::<&(EatsData, ApplesTag)>();

    let mut count = 0;
    q.each_entity_shared(&world, |e, ed| {
...
fn query_query_each_from_component() {
    ...
    let mut count = 0;
    {
        let qc = e_holder.get::<&QueryComp>().unwrap();
...
fn query_query_iter_from_component() {
    ...
    let mut count = 0;
    {
        let qc = e_holder.get::<&QueryIterComp>().unwrap();
...
fn query_field_at_from_each_w_iter() {
    ...
    let q = world.query::<&Position>().with(&mut Velocity::id()).build();
...
fn query_not_w_write() {
    ...
    q.each_entity_exclusive(&mut world, |_e, ()| {
        count += 1;
    });
...
fn query_iter_set_var_expr_in_place() {
    ...
    let mut count = 0;
    iter.each_entity_shared(&world, |e, p| {
        assert_eq!(e, e2);
        assert_eq!(p.x, 20);
        assert_eq!(p.y, 30);
        count += 1;
...
fn query_iter_entities() {
    ...
    world.new_query::<&Position>().run_shared(&world, |mut it| {
        while it.next() {
...
fn query_query_from_entity() {
    ...
    let mut count = 0;
    q1.each_entity_shared(&world, |e, _| {
        count += 1;
        assert_eq!(e, e2);
    });
    assert_eq!(count, 1);
...
fn query_query_from_entity_name() {
    ...
    let q1 = world.query::<(&Position, &Velocity)>().named("qe2").build();
...
fn query_add_to_match_from_staged_query() {
    ...
    stage.new_query::<&Position>().each_entity_shared(&world, |e, _| {
        e.add(Velocity::id());
        assert!(!e.has(Velocity::id()));
    });
...
fn query_optional_singleton() {
    ...
    world.new_query::<Option<&TestComp>>().each_exclusive(&mut world, |ptr| {
        assert!(ptr.is_none());
        invoked += 1;
    });
