...
fn sparse_get_get_mut() {
    ...
    {
        let v = e.get::<&TraitSparse>().unwrap();
        assert_eq!(v.x, 10.0);
        assert_eq!(v.y, 20.0);
    };

    {
        let mut mv = e.get::<&mut TraitSparse>().unwrap();
        mv.x = 30.0;
        mv.y = 40.0;
    };

    let found = e.get::<&TraitSparse>().map(|p| {
        assert_eq!(p.x, 30.0);
        assert_eq!(p.y, 40.0);
    });
...
fn sparse_try_get_not_found() {
    ...
    let e = world.entity();
    assert!(e.get::<&TraitSparse>().is_none());
    assert!(e.get::<&mut TraitSparse>().is_none());
}
...
fn dont_fragment_get_get_mut() {
    ...
    {
        let v = e.get::<&TraitDontFragment>().unwrap();
        assert_eq!(v.x, 10.0);
        assert_eq!(v.y, 20.0);
    };

    {
        let mut mv = e.get::<&mut TraitDontFragment>().unwrap();
        mv.x = 30.0;
        mv.y = 40.0;
    };

    let found = e.get::<&TraitDontFragment>().map(|p| {
        assert_eq!(p.x, 30.0);
        assert_eq!(p.y, 40.0);
    });
    assert!(found.is_some());
...
fn on_instantiate_override_get_get_mut() {
    ...
    {
        let v = e.get::<&TraitOnInstantiateOverride>().unwrap();
        assert_eq!(v.x, 10.0);
        assert_eq!(v.y, 20.0);
    };

    {
        let mut mv = e.get::<&mut TraitOnInstantiateOverride>().unwrap();
        mv.x = 30.0;
        mv.y = 40.0;
    };

    let found = e.get::<&TraitOnInstantiateOverride>().map(|p| {
        assert_eq!(p.x, 30.0);
        assert_eq!(p.y, 40.0);
    });
    assert!(found.is_some());
    ...
    assert!(
        world
            .entity()
            .get::<&TraitOnInstantiateOverride>()
            .is_none()
    );
}
...
fn on_instantiate_dont_inherit_get_get_mut() {
    ...
    {
        let v = e.get::<&TraitOnInstantiateDontInherit>().unwrap();
        assert_eq!(v.x, 10.0);
        assert_eq!(v.y, 20.0);
    };

    {
        let mut mv = e.get::<&mut TraitOnInstantiateDontInherit>().unwrap();
        mv.x = 30.0;
        mv.y = 40.0;
    };

    let found = e.get::<&TraitOnInstantiateDontInherit>().map(|p| {
        assert_eq!(p.x, 30.0);
        assert_eq!(p.y, 40.0);
    });
    assert!(found.is_some());
...
fn on_instantiate_inherit_get_inherited() {
    ...
    let found = inst.get::<&TraitOnInstantiateInherit>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    assert!(inst.get::<&mut TraitOnInstantiateInherit>().is_none());
}
...
fn no_traits_get_inherited() {
    ...
    let found = inst.get::<&TraitNone>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    assert!(inst.get::<&mut TraitNone>().is_none());
}
...
fn dynamic_inherit_dense_owned() {
    ...
    let found = e.get::<&TraitNone>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    let found = e.get::<&mut TraitNone>().map(|mut mp| {
        mp.x = 30.0;
    });
    assert!(found.is_some());
    {
        let p = e.get::<&TraitNone>().unwrap();
        assert_eq!(p.x, 30.0);
    };
}
...
fn static_inherit_dense_owned() {
    ...
    let found = e.get::<&TraitOnInstantiateInherit>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    let found = e.get::<&mut TraitOnInstantiateInherit>().map(|mut mp| {
        mp.x = 30.0;
    });
    assert!(found.is_some());
    {
        let p = e.get::<&TraitOnInstantiateInherit>().unwrap();
        assert_eq!(p.x, 30.0);
    };
}
...
fn static_inherit_sparse_owned() {
    ...
    let found = e.get::<&TraitSparseInherit>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    let found = e.get::<&mut TraitSparseInherit>().map(|mut mp| {
        mp.x = 30.0;
    });
    assert!(found.is_some());
    {
        let p = e.get::<&TraitSparseInherit>().unwrap();
        assert_eq!(p.x, 30.0);
    };
}
...
fn static_inherit_sparse_inherited() {
    ...
    let found = inst.get::<&TraitSparseInherit>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    assert!(inst.get::<&mut TraitSparseInherit>().is_none());
}
...
fn static_inherit_dont_fragment_owned() {
    ...
    let found = e.get::<&TraitDontFragmentInherit>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    let found = e.get::<&mut TraitDontFragmentInherit>().map(|mut mp| {
        mp.x = 30.0;
    });
    assert!(found.is_some());
    {
        let p = e.get::<&TraitDontFragmentInherit>().unwrap();
        assert_eq!(p.x, 30.0);
    };
}
...
fn static_inherit_dont_fragment_inherited() {
    ...
    let found = inst.get::<&TraitDontFragmentInherit>().map(|p| {
        assert_eq!(p.x, 10.0);
        assert_eq!(p.y, 20.0);
    });
    assert!(found.is_some());

    assert!(inst.get::<&mut TraitDontFragmentInherit>().is_none());
}
...
fn dynamic_inherit_sparse_owned() {
    ...
    {
        let _ = e.get::<&TraitSparse>().unwrap();
    };
}
...
fn dynamic_inherit_sparse_owned_get_mut() {
    ...
    {
        let _ = e.get::<&mut TraitSparse>().unwrap();
    };
}
...
fn dynamic_inherit_sparse_inherited() {
    ...
    {
        let _ = inst.get::<&TraitSparse>().unwrap();
    };
}
...
fn dynamic_inherit_dont_fragment_owned() {
    ...
    {
        let _ = e.get::<&TraitDontFragment>().unwrap();
    };
}
...
fn dynamic_inherit_dont_fragment_owned_get_mut() {
    ...
    {
        let _ = e.get::<&mut TraitDontFragment>().unwrap();
    };
}
...
fn dynamic_inherit_dont_fragment_inherited() {
    ...
    {
        let _ = inst.get::<&TraitDontFragment>().unwrap();
    };
}
